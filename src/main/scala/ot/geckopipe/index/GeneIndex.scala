...
abstract class GeneIndex extends Indexable {
  val biotypes = Set("3prime_overlapping_ncrna", "antisense", "IG_C_gene",
  ...
  def filterBiotypes(biotypes: Set[String]): GeneIndex =
    new GeneIndex {
      override val table: DataFrame =
        table.where(col("biotype") isin biotypes)
    }

  def sortByTSS: GeneIndex = new GeneIndex {
    override val table: DataFrame = table.sortWithinPartitions(col("chr").asc, col("tss").asc)
  }
  def sortByID: GeneIndex = new GeneIndex {
    override val table: DataFrame = table.sortWithinPartitions(col("chr").asc, col("gene_id").asc)
  }
}
...
object GeneIndex {
  /** A subset of all possible gene columns that can be included
    ...
    * }
    */
  val columns: Seq[String] = Seq("chr", "gene_id", "tss", "start", "end")
  ...
  def apply(from: String)(implicit ss: SparkSession): GeneIndex = {
    ...
    new GeneIndex {
      override val table: DataFrame = genes
    }
  }
}
