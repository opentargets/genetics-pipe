...
object Indexable {
  ...
  implicit class DataFrameIndexable(df: DataFrame) {
    ...
    /** select the data from table using fromSelect columns grouped by cols
      ...
      * and aggregate the result as first of each row resulting fromSelect diff cols = Seq("c3",
      * "c4")
      *
      * @param cols
      *   col names used to groupby
      * @param fromSelect
      *   col names used to select the initial table
      * @return
      *   a select grouped and aggregated by fromSelect diff cols columns
      */
