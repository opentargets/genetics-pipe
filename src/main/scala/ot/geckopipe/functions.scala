...
object functions extends LazyLogging {
  ...
  /** chromosomes is a map with default 0 that returns a Int from 1 to 25 from a chromosome string 1
    * to 22 and X Y MT consecutively numbered
    */
  ...
  val splitAndZip = udf((codes: String, labels: String) =>
    codes
      .split(";")
      .zipAll(labels.split(";"), "", "")
      .filter(_._1 != "")
  ...
  def loadFromCSV(uri: String, withSchema: StructType, andHeader: Boolean = true)(implicit
      ss: SparkSession
  ): DataFrame =
    ss.read
      .format("csv")
  ...
  def splitVariantID(
      ...
      withColTypes: List[String] = VariantIndex.columnsTypes
  ): Try[DataFrame] = {
  ...
  /** split by usingToken and grab the right side of the string path. Then split by '/'. It returns
    ...
    *
    * @param path
    *   input path to split by usingToken and then by '/'
    * @param usingToken
    *   the token used to partition the input path
    * @return
  ...
  def computePercentiles(ds: DataFrame, scoreField: String, percentileField: String)(implicit
      ss: SparkSession
  ): DataFrame = {
